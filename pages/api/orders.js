...
import OrderDublin from "@/models/order";
...
export default async function handler(req, res) {
  ...
  switch (method) {
    case "GET":
      try {
        const productos = await OrderDublin.find({}).sort({ createdAt: -1 });
    ...
    case "POST":
      try {
        const producto = new OrderDublin(req.body);
        await producto.save();
    ...
    case "PUT":
      try {
        ...
        const producto = await OrderDublin.findByIdAndUpdate(_id, data, {
          new: true,
        });
